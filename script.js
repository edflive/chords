...
function calculateVoicingPosition(voicing) {
    ...
    for (const fret of voicing.frets) {
        if (fret > 0) {
            foundFrettedNote = true; minFret = Math.min(minFret, fret);
        }
    }
...
function calculateTransitionCost(voicing1, voicing2) {
    ...
    for (let i = 0; i < DIAGRAM_STRINGS; i++) {
        ...
        if (fret1 === -1 && fret2 === -1) {
            continue; // cordes muettes dans les deux cas → aucun coût
        }

        if (finger1 > 0 && finger1 === finger2 && fret1 === fret2) {
            cost += 0; // même doigt, même case → aucun coût
        } else if (fret1 !== -1 && fret2 === -1) {
            cost += 3; // doigt posé puis corde étouffée → coût de disparition
        } else if (fret1 === -1 && fret2 !== -1) {
            cost += 3; // corde muette puis doigt posé → coût d’apparition
        } else {
            cost += Math.abs(fret1 - fret2); // déplacement longitudinal
            if (finger1 !== finger2 || fret1 === 0 || fret2 === 0) {
                cost += 2; // changement de doigt ou apparition/disparition d'une corde à vide
            }
        }
    }
...
function findAlternate(currentVoicing, currentChordId, direction) {
    ...
    let targetIndex;
    if (direction === 1) {
        // Voicing suivant avec rotation (si à la fin, retourne le premier)
        targetIndex = (currentIndex + 1) % allVoicings.length;
    } else {
        // Voicing précédent avec rotation (si au début, retourne le dernier)
        targetIndex = (currentIndex - 1 + allVoicings.length) % allVoicings.length;
    }

    /* console.log(`findNextVoicing (direction ${direction}): switching from index ${currentIndex} to index ${targetIndex}`); */
...
function drawFretboardVisualization(voicingData, parentElement, pivotIndices = new Set()) {
    ...
    // Marqueurs de repère (frettes 3, 5, 7, 9)
...
altDownButton.addEventListener('click', () => {
    ...
    if (alternate) {
        ...
        /* console.log("Pas d'autre accord trouvé avant."); */
    }
});
...
altUpButton.addEventListener('click', () => {
    ...
    if (alternate) {
        ...
        /* console.log("Pas d'autre accord trouvé à suivre."); */
    }
});
